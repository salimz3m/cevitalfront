...
export default function PlanifIntelligent() {
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch(() => {});
    api
      .get("/planification/sessions")
      .then(({ data }) => setSessions(data.filter((s) => s.lignes?.length > 0)))
      .catch(() => {});
    api
  ...
  // Remplacer handleSuggestion par :
  const handleSuggestion = async () => {
    if (!suggClrId) return;
    setSuggLoading(true);
    setSuggResult(null);
    try {
      const url = suggOrderId
        ? `/modules/planning-intel/suggestion-diapason/${suggClrId}?orderId=${suggOrderId}`
        : `/modules/planning-intel/suggestion-diapason/${suggClrId}`;
  ...
  const handleOptimisation = async () => {
    ...
    try {
      const { data } = await api.get(
        `/modules/planning-intel/optimisation/${optimSessionId}`,
      );
      setOptimResult(data);
    } catch {
    } finally {
  ...
  // ── RENDER ──────────────────────────────────────────────────
  return (
    <ModuleGate module="PLANIF_INTEL">
      <div className="ds-page">
        ...
        <div className="ds-header">
          ...
          <h1 className="ds-title">
            PLANNING <span>INTELLIGENT</span>
          </h1>
          <p className="ds-subtitle">
            Analyse en temps réel — suggestions diapason — alertes rupture —
            simulation de flux
          </p>
        </div>
        ...
        <div className="pi-tabs">
          {[
            ...
            >
              ...
              {t.id === "alertes" &&
                resume &&
                resume.totalOrange + resume.totalRouge > 0 && (
                  <span
                    style={{
                      marginLeft: 6,
                      background: "#ef4444",
                    ...
                    {[
                      {
                        cls: "green",
                        nb: resume.totalVert,
                        lbl: "CLR en bonne santé",
                      },
                      {
                        cls: "orange",
                        nb: resume.totalOrange,
                        lbl: "CLR en attention",
                      },
                      {
                        cls: "red",
                        nb: resume.totalRouge,
                        lbl: "CLR en rupture",
                      },
                      {
                        cls: "grey",
                        nb: resume.totalNeutre,
                        lbl: "CLR sans activité",
                      },
                    ].map((s) => (
                    ...
                    <div key={region}>
                      ...
                      <div className="pi-grid-clrs">
                        {items.map((d) => (
                          <div
                            key={d.clr.id}
                            className={`pi-clr-card ${d.couleur}`}
                          >
                            <div className="pi-clr-code">
                              <span
                                className={`pi-niveau-dot ${d.couleur}`}
                              ></span>
                              {d.clr.code}
                            </div>
                            <div className="pi-clr-nom">
                              {d.clr.nom} — {d.clr.wilaya}
                            </div>
                            <div className="pi-clr-stat">
                              Stock : {d.stockActuel.toLocaleString()} u
                            </div>
                            <div className="pi-clr-stat">
                              Commandes : {d.commandesDues.toLocaleString()} u
                            </div>
                ...
                {loadingAlertes ? (
                  ...
                  <div className="ds-empty" style={{ padding: 40 }}>
                    ...
                    <p style={{ color: "#10b981", fontWeight: 700 }}>
                      Aucune alerte — tous les CLR sont en ordre
                    </p>
                  </div>
                ) : (
                  <>
                    ...
                    >
                      {alertes.nbAlertes} alerte(s) — analyse du{" "}
                      {new Date(alertes.dateAnalyse).toLocaleString("fr-DZ")}
                    </div>
                      ...
                      <div key={i} className={`pi-alert-card ${a.urgence}`}>
                        ...
                        <div>
                          <div className="pi-alert-title">
                            {a.clr.code} — {a.clr.nom}
                          </div>
                          <div className="pi-alert-sub">
                            Stock actuel : {a.stockActuel} u · Commandes dues :{" "}
                            {a.commandesDues} u · Écart : {a.ecart} u
                          </div>
                          <div className="pi-alert-sub">{a.message}</div>
                          <div className={`pi-alert-action ${a.urgence}`}>
                            → {a.action}
                          </div>
                        </div>
                      </div>
          ...
          <div>
            <div className="ds-panel">
              ...
              <div style={{ padding: "20px 24px" }}>
                ...
                >
                  Sélectionnez un CLR et optionnellement une commande pour
                  croiser le stock disponible avec les produits demandés.
                </p>
                ...
                >
                    ...
                    >
                      ...
                      {orders.map((o) => (
                        <option key={o.id} value={o.id}>
                          {o.orderNumber} —{" "}
                          {new Date(o.date).toLocaleDateString("fr-DZ")}
                        </option>
                      ))}
                    </select>
                  </div>
                  ...
                  <button
                    ...
                    disabled={!suggClrId || suggLoading}
                  >
                    {suggLoading ? (
                      "Analyse…"
                    ) : (
                ...
                {suggResult && (
                  <div className="pi-diap-result">
                    ...
                    <div style={{ marginBottom: 16 }}>
                      ...
                      <div className="pi-diap-suggestion">
                        DIAPASON {suggResult.suggestion}
                      </div>
                      <div className="pi-diap-raison">{suggResult.raison}</div>
                      <div className="pi-diap-score">
                        Score de confiance : {suggResult.score}%
                      </div>
                    </div>
                    ...
                    >
                      {[
                        {
                          lbl: "Stock CLR actuel",
                          val: suggResult.stockActuelCLR + " u",
                        },
                        {
                          lbl: "Stock plateforme",
                          val:
                            suggResult.stockPlateforme > 0
                              ? suggResult.stockPlateforme + " u"
                              : "N/A",
                        },
                        {
                          lbl: "Livraisons actives",
                          val: suggResult.nbLignesActives,
                        },
                      ].map((s) => (
                        <div
                          key={s.lbl}
                    ...
                    {/* ── NOUVEAU : stock par produit croisé commande ── */}
                    {suggResult.stockParProduit?.length > 0 && (
                      <div style={{ marginBottom: 16 }}>
                        <div
                          style={{
                            fontSize: ".65rem",
                        ...
                        >
                          COUVERTURE STOCK PAR PRODUIT
                        </div>

                        {/* Résumé couverture */}
                        {suggResult.resumeCouverture && (
                          <div
                            style={{
                              display: "flex",
                              gap: 10,
                          ...
                          >
                            {[
                              {
                                lbl: "Couverts",
                                nb: suggResult.resumeCouverture.couverts,
                                color: "#10b981",
                              },
                              {
                                lbl: "Partiels",
                                nb: suggResult.resumeCouverture.partiels,
                                color: "#f59e0b",
                              },
                              {
                                lbl: "Impossibles",
                                nb: suggResult.resumeCouverture.impossibles,
                                color: "#ef4444",
                              },
                            ].map((r) => (
                              <div
                        ...
                        >
                          ...
                          <tbody>
                            {suggResult.stockParProduit.map((p, i) => (
                              <tr
                                key={i}
                                style={{
                              ...
                              >
                                ...
                                <td
                                  style={{
                                    padding: "8px 10px",
                                    color: p.manque > 0 ? "#ef4444" : "#aaa",
                                    fontWeight: p.manque > 0 ? 700 : 400,
                                  }}
                                >
                                  {p.manque > 0 ? `−${p.manque} u` : "—"}
                                </td>
                                <td style={{ padding: "8px 10px" }}>
                                  <div
                                    style={{
                    ...
                    <div>
                      ...
                      >
                        {Object.entries(suggResult.alternatives).map(
                          ([d, alt]) => {
          ...
          <div>
            <div className="ds-panel">
              ...
              <div style={{ padding: "20px 24px" }}>
                ...
                >
                  Simulez l'impact d'un envoi sur le stock CLR et la plateforme
                  avant de planifier.
                </p>
                ...
                <div className="pi-simul-form">
                  <div className="ds-field" style={{ marginBottom: 0 }}>
                    ...
                    <select
                      ...
                      value={simForm.clrId}
                      onChange={(e) =>
                        setSimForm({ ...simForm, clrId: e.target.value })
                      }
                    >
                  ...
                  <div className="ds-field" style={{ marginBottom: 0 }}>
                    ...
                    <input
                      ...
                      min="1"
                      placeholder="Ex: 150"
                      value={simForm.quantite}
                      onChange={(e) =>
                        setSimForm({ ...simForm, quantite: e.target.value })
                      }
                    />
                  </div>
                  <div className="ds-field" style={{ marginBottom: 0 }}>
                    ...
                    <select
                      className="ds-input"
                      value={simForm.diapason}
                      onChange={(e) =>
                        setSimForm({ ...simForm, diapason: e.target.value })
                      }
                    >
                      <option value="D1">D1 — Plateforme → CLR</option>
                      <option value="D2">D2 — Usine → CLR (direct)</option>
                      <option value="D3">D3 — CLR → CLR (transfert)</option>
                      <option value="D4">D4 — Usine → Plateforme</option>
                      <option value="D5">D5 — Retour CLR → Plateforme</option>
                    </select>
                  </div>
                  <button
                    className="ds-btn ds-btn-dark"
                    onClick={handleSimuler}
                    disabled={!simForm.clrId || !simForm.quantite || simLoading}
                  >
                    {simLoading ? (
                      "…"
                    ) : (
                ...
                {simResult && (
                  <div>
                    ...
                    {!simResult.faisable && (
                      ...
                      >
                        ❌ Simulation non faisable — stock plateforme
                        insuffisant
                      </div>
                    )}
                    <div className="pi-simul-result">
                      <div>
                        ...
                        <div className="pi-simul-bloc">
                          <div style={{ marginBottom: 10 }}>
                            ...
                            <div className="pi-simul-val">
                              {simResult.avant.stockCLR} u
                            </div>
                          </div>
                          {simForm.diapason === "D1" && (
                            <div>
                              <div className="pi-simul-label">
                                Stock Plateforme
                              </div>
                              <div className="pi-simul-val">
                                {simResult.avant.stockPlateforme} u
                              </div>
                            </div>
                          )}
                        </div>
                      ...
                      <div>
                        <div className="pi-simul-label">
                          APRÈS (+{simForm.quantite} u via {simForm.diapason})
                        </div>
                        <div className="pi-simul-bloc apres">
                          <div style={{ marginBottom: 10 }}>
                            <div className="pi-simul-label">Stock CLR</div>
                            <div
                              className="pi-simul-val"
                              style={{ color: "#10b981" }}
                            >
                              {simResult.apres.stockCLR} u
                            </div>
                            <div className="pi-simul-sub">
                              +{simResult.impact.gainCLR} u
                            </div>
                          </div>
                          {simForm.diapason === "D1" && (
                            <div>
                              ...
                              <div
                                ...
                                style={{
                                  color:
                                    simResult.apres.stockPlateforme <
                                    simResult.avant.stockPlateforme
                                      ? "#ef4444"
                                      : "#333",
                                }}
                              >
                              ...
                              <div className="pi-simul-sub">
                                -{simResult.impact.pertePlateforme} u
                              </div>
                            </div>
                          )}
                        </div>
          ...
          <div>
            <div className="ds-panel">
              ...
              <div style={{ padding: "20px 24px" }}>
                ...
                >
                  Analysez une session de planification pour optimiser le
                  regroupement des livraisons et le remplissage des camions.
                </p>
                <div
                  style={{
                    display: "flex",
                ...
                >
                  ...
                  >
                    ...
                    <select
                      className="ds-input"
                      value={optimSessionId}
                      onChange={(e) => {
                        setOptimSessionId(e.target.value);
                        setOptimResult(null);
                      }}
                    >
                      ...
                      {sessions.map((s) => (
                        <option key={s.id} value={s.id}>
                          Session #{s.id} — {s.date} ({s.lignes?.length || 0}{" "}
                          lignes)
                        </option>
                      ))}
                    </select>
                  ...
                  <button
                    ...
                    disabled={!optimSessionId || optimLoading}
                  >
                    {optimLoading ? (
                      "Analyse…"
                    ) : (
                ...
                {optimResult && (
                  <div>
                    ...
                    >
                      {optimResult.nbGroupes} groupe(s) CLR · Capacité camion :{" "}
                      {optimResult.capaciteCamion} u
                    </div>
                    {optimResult.suggestions.length === 0 ? (
                      <div className="ds-empty">
                        <p>Aucune donnée à optimiser</p>
                      </div>
                    ) : (
                      optimResult.suggestions.map((s, i) => (
                        <div key={i} className="pi-optim-card">
                          ...
                          <div>
                            <div className="pi-optim-reco">
                              {s.recommandation}
                            </div>
                            <div className="pi-optim-sub">
                              {s.nbLignes} ligne(s) · {s.quantiteTotale} u ·
                              Taux : {s.tauxRemplissage}%
                            </div>
                            {s.produitsDetail?.length > 0 && (
                              <div
                                style={{
                                  marginTop: 6,
                                  display: "flex",
                              ...
                              >
                                {s.produitsDetail.map((p, pi) => (
                                  ...
                                  >
                                    {p.sku || p.nom} · {p.quantite} u
                                  </span>
                                ))}
                              </div>
                            )}
                          </div>
                          <div className="pi-optim-badge">
                            {s.nbCamionsNeeded} camion
                            {s.nbCamionsNeeded > 1 ? "s" : ""}
                          </div>
                        </div>
                      ))
                    )}
